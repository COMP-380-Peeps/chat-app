...
const ChatBox = () => {
    ...
    const [bannedUsers, setBannedUsers] = useState<string[]>([]); // Define a state for banned users
            ...
            getDocs(serverQuery)
                .then((serverSnapshot) => {
                    ...
                    const unsubscribeMessages = onSnapshot(messagesQuery, (querySnapshot) => {
                        ...
                        // Filter out messages sent by banned users
                        const filteredMessagesData = messagesData.filter((message) => !bannedUsers.includes(message.sender_id));
                        setMessages(filteredMessagesData.sort((a, b) => a.created_at - b.created_at));
                    })
    ...
    }, [currentUser, isWhitelisted, bannedUsers])

    const handleBanUser = (username: string) => {
        setBannedUsers([...bannedUsers, username.toLowerCase()]);
    };
    ...
    if (currentUser && isWhitelisted) {
        return (
                ...
                <div>
                    <button onClick={() => handleBanUser("username1")}>Ban username1</button>
                    <button onClick={() => handleBanUser("username2")}>Ban username2</button>
                    <button onClick={() => setBannedUsers([])}>Clear bans</button>
                </>
            </ul>
        );
    } else {
